...
geraAGBase(Data, Camiao, MelhorPlaneamento,Tempo, NG, DP, P1, P2, TMax, LimEstabilizacao):-
	inicializaARQSI(NG,DP,P1,P2),
	gera_populacaoAGBase(Pop, Data, Camiao),
	write('Pop='),write(Pop),nl,
	avalia_populacao(Pop,PopAv,Data,Camiao),
	write('PopAv='),write(PopAv),nl,
	ordena_populacao(PopAv,PopOrd),
	geracoes(NG),
	get_time(Ti),
	gera_geracaoAGBase(0,NG,PopOrd,0,LimEstabilizacao,Ti,TMax,0,Data,Camiao,MelhorPlaneamento*Tempo).
...
gera_populacaoAGBase(Pop, Data, Camiao):-
	...
	length(ListaEntregas, NumE),
	(retract(entregas(_));true),	asserta(entregas(NumE)),
	gera_populacaoAGBase(TamPop,ListaEntregas,NumE,Pop, Data, Camiao).


gera_populacaoAGBase(0,_,_,[],Data,Camiao):-!.

gera_populacaoAGBase(TamPop,ListaEntregas,NumE,[Ind|Resto],Data,Camiao):-
	TamPop1 is TamPop-1,
	gera_populacaoAGBase(TamPop1,ListaEntregas,NumE,Resto,Data,Camiao),
	gera_individuo(ListaEntregas,NumE,Ind),
	not(member(Ind,Resto)).

gera_populacaoAGBase(TamPop,ListaEntregas,NumE,L,Data,Camiao):-
	gera_populacaoAGBase(TamPop,ListaEntregas,NumE,L,Data,Camiao).

gera_geracaoAGBase(G,G,Pop,_,_,_,_,_,_,_,MelhorPlaneamento):-!,
	write('Geracao '), write(G), write(':'), nl, write(Pop), nl,
	obterMelhorPlaneamento(Pop,MelhorPlaneamento).

gera_geracaoAGBase(G,_,Pop,LimEstabilizacao,LimEstabilizacao,_,_,_,_,_,MelhorPlaneamento):-!,
	write('Geracao '), write(G), write(':'), nl, write(Pop), nl,
	obterMelhorPlaneamento(Pop,MelhorPlaneamento).

gera_geracaoAGBase(G,_,Pop,_,_,_,_,1,_,_,MelhorPlaneamento):-!,
	write('Geracao '), write(G), write(':'), nl, write(Pop), nl,
	obterMelhorPlaneamento(Pop,MelhorPlaneamento).

gera_geracaoAGBase(N,G,Pop,Cont,LimEstabilizacao,Ti,TMax,_,Data,Camiao,MelhorPlaneamento):-
	write('Geracao '), write(N), write(':'), nl, write(Pop), nl,
	cruzamento(Pop,NPop1),
	mutacao(NPop1,NPop),
	avalia_populacao(NPop,NPopAv,Data,Camiao),
	ordena_populacao(NPopAv,NPopOrd),
	verificarEstabilizacao(Pop,NPopOrd,Cont,ContAtualizado),
	N1 is N+1,
	get_time(Tf),
	TSol is Tf-Ti,
	verificarTempoLimite(TSol,TMax,FlagMax),
	gera_geracaoAGBase(N1,G,NPopOrd,ContAtualizado,LimEstabilizacao,Ti,TMax,FlagMax,Data,Camiao,MelhorPlaneamento).